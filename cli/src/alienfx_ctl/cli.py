...
def cmd_udev_rule(args) -> int:
    ...
    Generated rather than shipped, because a static rule can only name the
    product ids of the machine it was written on. Detection matches on vendor
    id and HID report shape, so it recognises a controller whose product id
    differs; the rule then has to name that id or the ACL is never granted and
    the plugin finds the device but cannot open it.

    Reads only sysfs, which is world readable, so this runs before the rule
    exists and without any access to the device node itself.

    Exits non-zero when nothing is found, so the installer can fall back to the
    bundled rule rather than writing an empty one.
    """
    lines, found = [], []
    for controller in device.CONTROLLERS:
        ...
        lines.append(f"# {controller.label}")
        lines.append(
            f'SUBSYSTEM=="hidraw", ATTRS{{idVendor}}=="{vid:04x}", '
            f'ATTRS{{idProduct}}=="{pid:04x}", TAG+="uaccess"')

    if not found:
        return _fail("no lighting controllers found; cannot generate a rule", 1)

    print("# Omarchy AlienFX plugin - grant the logged-in seat user direct access")
    print("# to this machine's lighting controllers, so nothing needs sudo again.")
    print("#")
    print("# Generated by `alienfx-ctl udev-rule` from the hardware present. Product")
    print("# ids differ between models, so a rule written for one machine grants")
    print("# nothing on another.")
    print("#")
    print("# TAG+=\"uaccess\" asks systemd-logind for an ACL for whoever is logged")
    print("# into the local seat. It sets no mode and adds no group.")
    print("#")
    print("# The 60- prefix is load-bearing. The ACL is applied by the uaccess")
    print("# builtin invoked from 73-seat-late.rules, which only sees tags already")
    print("# set when it runs. A rule numbered above 73 is read too late and the")
    print("# node stays root-only, which looks exactly like a boot-time race.")
    for line in lines:
        print(line)
    return 0
