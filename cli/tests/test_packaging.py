...
def test_the_cli_runs_the_uninstaller_from_a_copy_not_in_place():
    """bash reads a script as it goes, and the uninstaller deletes the
    directory it is sitting in. Running it in place risks it being read out
    from under itself."""
    import inspect
...
def test_the_uninstaller_goes_through_that_one_path():
    import inspect
    from alienfx_ctl import cli
    assert "_run_bundled_script" in inspect.getsource(cli.cmd_uninstall)


def test_nothing_ships_a_self_update_path():
    """The marketplace review rejected an updater that fetched whatever the
    branch currently held and then executed the installer out of it. The
    version check stays, but nothing here may fetch code and run it: that is a
    listing blocker, not a style preference."""
    import os
    root = os.path.join(os.path.dirname(__file__), "..", "..")
    assert not os.path.exists(os.path.join(root, "update.sh"))
    from alienfx_ctl import cli
    assert not hasattr(cli, "cmd_update")
    for name in ("install.sh", "uninstall.sh"):
        script = _read(name)
        assert "omarchy plugin update" not in script, name
