...
Open the popup and the version sits next to the title. It is dim and says nothing most of the
time. When a newer release has been tagged it takes the theme's accent colour and an info mark
appears beside it, pulsing slowly. Click it to open the plugin's page on the official Omarchy
marketplace, where the listing shows the exact commit that was validated and its verification
state.

**The plugin never updates itself.** It does not download code, and it does not run an
installer on your behalf. Updating is Omarchy's own command, run by you:

```bash
omarchy plugin update 3mrgnc3.alienfx
```

That shows you the full diff and asks before applying anything. It moves the plugin folder
only, so afterwards the popup will say **Update needs finishing**: the CLI, the user services
and the theme hook live outside that folder. Clicking it runs the installer already sitting in
the plugin folder, which lists what it will change and asks first. The udev rule is left alone
unless it has actually changed, so that step usually needs no password.
...
repository you cloned from and reads the list of release tags. That is a request for a list of
names: no code is downloaded, nothing is executed, and nothing about you or your machine is
sent. The answer is kept for a day, so reopening the popup costs nothing, and there is no
timer, so nothing happens while the popup is shut.
...
The rule is generated at install time from the controllers actually present, because
product ids differ between models and a rule naming one machine's ids grants nothing on
another. `alienfx-ctl udev-rule` prints it; the installer writes it.
...
cli/tests/                             624 tests, no hardware required
...
install.sh  uninstall.sh             everything outside the plugin folder
```
...
```bash
cd cli && python3 -m pytest tests -q     # 624 tests, no hardware needed
```
